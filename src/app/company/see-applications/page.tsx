...
export default function SeeApplicatios() {
    ...
    try {
        ...
        onSnapshot(q, async (snapshot) => {
            const job = snapshot.docs.map((job) => {
                return {jobId: job.data().jobId, id: job.id, coverLetter: job.data().coverLetter, userId: job.data().uid}
            })
            ...
            const jobPromises  = job.map(async (job) => {
                ...
                return {...getJob.data(), id: job.id, coverLetter: job.coverLetter, user:{...getUser.data()}}
            })
